const urlBase = "https://jsonplaceholder.typicode.com/posts"; // URL Main
let posts = []; // Array for posts
...
function getData() {
  fetch(urlBase)
    ...
    .catch((error) => console.error("Error calling to the API: ", error));
}
...
function postData() {
  ...
  fetch(urlBase, {
    ...
    .catch((error) => console.error("Erorr to create post: ", error));
}
  ...
  fetch(`${urlBase}/${id}`, {
    ...
    .then(data => {
        ...
        if(index != -1){
            ...
            alert('Error in Update Post')
        }
        renderPostList();
    })
    .catch((error) => console.error("Erorr to update post: ", error));
}
